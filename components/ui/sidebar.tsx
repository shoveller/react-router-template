...
// 유틸리티 함수
function createOpenStateFromValue(
  value: boolean | ((value: boolean) => boolean),
  currentOpen: boolean
): boolean {
  if (typeof value === 'function') {
    return value(currentOpen)
  }
...
function handleSidebarState(
  ...
  localSetOpen: React.Dispatch<React.SetStateAction<boolean>>
): void {
  if (setOpenProp) {
  ...
  localSetOpen(openState)
}

function saveSidebarStateToCookie(openState: boolean): void {
...
function getCollapsibleAttr(state: string, collapsible: string): string {
  if (state === 'collapsed') {
    return collapsible
  }
...
>(
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        // 값에서 상태 계산하기
        const openState = createOpenStateFromValue(value, isOpen)

        // 로컬 또는 외부 상태 업데이트
        handleSidebarState(openState, setOpenProp, _setOpen)

        // 쿠키에 상태 저장
        saveSidebarStateToCookie(openState)
      },
      [setOpenProp, isOpen]
    )
    ...
    const toggleSidebar = React.useCallback(() => {
      if (isMobile) {
        handleMobileToggle(setOpenMobile)

        return
      }

      handleDesktopToggle(setOpen)
    ...
    const state = getSidebarState(isOpen)
...
>(
  (
    ...
    const collapsibleValue = getCollapsibleAttr(state, collapsible)

    // 첫 번째 div 클래스 이름 결정
    const firstDivClassName = getFirstDivClassNameByVariant(variant)

    // 사이드바 클래스이름 결정
    const sideClassName = getSideClassNameBySide(side)

    // 바리언트 클래스이름 결정
    const variantClassName = getVariantClassNameByVariant(variant)
...
>(({ className, asChild = false, ...props }, ref) => {
  const Comp = getSlotOrDefault(asChild, 'div')
  ...
  return (
    <Comp
      ...
      className={cn(
        `duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs
...
>(({ className, asChild = false, ...props }, ref) => {
  const Comp = getSlotOrDefault(asChild, 'button')
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      ...
      px-1 text-xs font-medium tabular-nums text-sidebar-foreground select-none
      pointer-events-none`,
...
>(({ asChild = false, size = 'md', isActive, className, ...props }, ref) => {
  const Comp = getSlotOrDefault(asChild, 'a')
...
// State for data attribute
function getSidebarState(isOpen: boolean): 'expanded' | 'collapsed' {
  if (isOpen) {
    return 'expanded'
  }
...
// 첫 번째 div 클래스 이름 결정 함수
function getFirstDivClassNameByVariant(variant: string): string {
  if (variant === 'floating' || variant === 'inset') {
    return getFloatingOrInsetFirstDivClass()
  }

  return getDefaultFirstDivClass()
}

// 사이드바 클래스이름 결정 함수
function getSideClassNameBySide(side: string): string {
  if (side === 'left') {
    return getLeftSideClass()
  }

  return getRightSideClass()
}

// 바리언트 클래스이름 결정 함수
function getVariantClassNameByVariant(variant: string): string {
  if (variant === 'floating' || variant === 'inset') {
    return getFloatingOrInsetClass()
  }

  return getDefaultVariantClass()
}
